<template>
     <div class="header">
      ...
      <ul class="navigation">
        ...
        <li class="navigation_item someitem" @mouseenter="showitem" @mouseleave="hideitem">WhitePaper
          <img src="@/components/common/images/arrow.png" class="white-arrow">
          <ul class="white-summary" v-if="show">
            <li class="white" @click="OpenWhitePaper()">Whitepaper</li>
            <li class="white" @click="OpenPDF()">Executive Summary</li>
          </ul>
        </li>
...
export default {
  ...
  methods: {
    ...
    OpenWhitePaper () {
      ...
      // window.open(location.href + '?action=submitemail', '_blank')
    ...
    OpenWallet () {
      this.$ga.event('viewWallet', 'click', 'wallet')
      window.open('https://www.topnetwork.org/testnet/#/testnet/wallet', '_blank')
    },
    OpenExplorer () {
      this.$ga.event('viewExplorer', 'click', 'explorer')
